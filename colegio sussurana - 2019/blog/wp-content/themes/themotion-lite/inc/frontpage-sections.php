...
if ( ! function_exists( 'themotion_has_three_videos_section' ) ) :
	...
	function themotion_has_three_videos_section() {
		...
		if ( isset( $themotion_show_videos ) && true !== $themotion_show_videos ) {
			...
			if ( ! empty( $themotion_video_category ) && 'all' !== $themotion_video_category ) {
				$args['category_name'] = $themotion_video_category;
			}
...
if ( ! function_exists( 'themotion_display_posts_on_b' ) ) :

	/**
	 * Function to show posts on template b.
	 *
	 * @param string $category  Category of posts.
	 * @param int    $post_nb      Number of posts to display.
	 * @param bool   $is_callback See if it's called from ajax or in template file.
	 */
	function themotion_display_posts_on_b( $category = '', $post_nb = '', $is_callback = false ) {

		$themotion_home_b_post_nb = empty( $post_nb ) ? get_theme_mod( 'themotion_home_b_post_nb', 3 ) : $post_nb;
		if ( $themotion_home_b_post_nb === 0 ) {
			if ( is_customize_preview() ) { ?>
				<section class="posts-on-b"></section>
				<?php
			}
		} else {

			if ( $is_callback === false ) {?>
				<section class="posts-on-b">
				<?php
			} ?>
				<div class="container">
					<div class="content-wrap">
						...
						$themotion_bottom_posts_category = empty( $category ) ? get_theme_mod( 'themotion_bottom_posts_category' ) : $category;
						$args = array(
							'post_type' 		=> 'post',
							'post_status' 		=> 'publish',
							'posts_per_page' 	=> ( ! empty( $themotion_home_b_post_nb ) ? (int) $themotion_home_b_post_nb : '3' ),
